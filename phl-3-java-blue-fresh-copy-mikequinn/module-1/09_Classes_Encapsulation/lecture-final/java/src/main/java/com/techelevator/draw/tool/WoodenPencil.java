...
public class WoodenPencil {
	...
	// constants
	//
	// Please note that this sample code does not actually follow the convention
	// for naming constants in java. Based on the java naming conventions,
	// constants should be all caps with underscores between words. That would mean
	// defaultLength should actually be DEFAULT_LENGTH and so on.
	//
	// This is the convention for constants: ALL CAP separated by underscore
	 ...
	 * INSTANCE VARIABLES
	 * 
	 * Each instance of a wooden pencil has the individual attributes below.
	 * 
	 * NOTE that we declare them private so that only the instance has access to these
	 * 
	 */
     ...
     * 
     * Getter
     *   starts with 'get' + instance variable name. The final name should be
     *   camel case (so color becomes getColor even though color starts with a lowercase
     *   letter).
     * 
     *   no parameters
     ...
     *   
     * Setter
     *   starts with 'set'  + instance variable name. The final name should be
     *   camel case (so color becomes setColor even though color starts with a lowercase
     *   letter).
	...
	/*
	 * Note that getters/setters can be derived. In other words, we can
	 * make getters or setters that which are not named based on a variable name.
	 * These methods may work with other properties but there does not need
	 * to be a property that matches the name (i.e. isStub below indicates if
	 * checks if the pencil is currently a stub. it figures that out based
	 * on stubLength but there is no property called stub). 
	 */
