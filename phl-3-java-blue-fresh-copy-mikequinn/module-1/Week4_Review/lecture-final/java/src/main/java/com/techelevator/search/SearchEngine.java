...
public class SearchEngine {
	...
	public void indexFiles() throws SearchEngineException, Exception {
		...
		// Retrieve the list of filepaths from the SearchDomain
		List<String> filePaths = sd.getFiles();
		
		// Loop through each filepaths and read files
		// We use for rather than for-each because we 
		// need an index to pass to indexWords as a FileID
		for (int i = 0; i < filePaths.size(); i++) {
			// make File from filepath
			File filePath = new File(filePaths.get(i));
			
			// create the Scanner using the File with try-with-resource
			// so it gets cleaned up
			try (Scanner fileContents = new Scanner(filePath)) {
				// Loop through all lines
				while(fileContents.hasNextLine()) {
					// call indexWords with file id and current line
					indexWords(i, fileContents.nextLine());
				}
			}
		}
