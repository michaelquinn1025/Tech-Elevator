...
public class HotelService {
  ...
  public Reservation addReservation(String newReservation) throws HotelServiceException {
    ...
    try {
	    ResponseEntity<Reservation> response = restTemplate.exchange(BASE_URL + "/hotels/" + reservation.getHotelID() + "/reservations",
	    		HttpMethod.POST, makeReservationEntity(reservation), Reservation.class);
	    
	    return response.getBody();
  	} catch(RestClientResponseException ex) {
  		String message = "Error " + ex.getRawStatusCode() + ": " + ex.getStatusText();
  		throw new HotelServiceException(message);
  	}
...
//      hotels = restTemplate.exchange(BASE_URL + "hotels", HttpMethod.GET, makeAuthEntity(), Hotel[].class).getBody();
    	ResponseEntity<Hotel[]> responseEntity = restTemplate.exchange(BASE_URL + "hotels", HttpMethod.GET, makeAuthEntity(), Hotel[].class);
    	hotels = responseEntity.getBody();
    } catch (RestClientResponseException ex) {
      throw new HotelServiceException(ex.getRawStatusCode() + " : " + ex.getResponseBodyAsString());
    }
    return hotels;
