...
public class SearchDomain {
	...
	private List<String> buildDomain() throws SearchDomainException {
		...
		File dirToBuildFrom = new File(this.folder);
		
		if (dirToBuildFrom.exists() && dirToBuildFrom.isDirectory()) {
			File[] filesInDir = dirToBuildFrom.listFiles();/*
			...
			*/
			...
			// for-each is preferable when it works for your needs
			...
			throw new SearchDomainException("File path " + this.folder 
